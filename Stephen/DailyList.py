...
def StartDailyGeneration():
    ...
    NormalX = preprocessing.Normalizer().fit(X)
    X = NormalX.transform(X)


    
    # Loads Pre-Created Model
    loadedModel1 = pickle.load(open("sklearnModels/LinearRegression_Model.sav", 'rb'))
    loadedModel2 = pickle.load(open("sklearnModels/RandomForestRegressor.sav", 'rb'))
    loadedModel3 = pickle.load(open("sklearnModels/adaBoost.sav", 'rb'))
    loadedModel4 = pickle.load(open("sklearnModels/bagging.sav", 'rb'))
    loadedModel5 = pickle.load(open("sklearnModels/extraTree.sav", 'rb'))
    loadedModel6 = pickle.load(open("sklearnModels/GradientBoosting.sav", 'rb'))
    loadedModel7 = pickle.load(open("sklearnModels/HistGradient.sav", 'rb'))
    loadedModel8 = pickle.load(open("sklearnModels/PLSRegression.sav", 'rb'))
    loadedModel9 = pickle.load(open("sklearnModels/DummyRegressor.sav", 'rb'))

    # Apply Pro-Created Model
    predictions1 = loadedModel1.predict(X)
    predictions2 = loadedModel2.predict(X)
    predictions3 = loadedModel3.predict(X)
    predictions4 = loadedModel4.predict(X)
    predictions5 = loadedModel5.predict(X)
    predictions6 = loadedModel6.predict(X)
    predictions7 = loadedModel7.predict(X)
    predictions8 = loadedModel8.predict(X)
    predictions9 = loadedModel9.predict(X)

    # Creates list of Predictions and Tickers for sorting
    tickerPredictList = []
    for i in range(len(predictions1)):
        currentItem = [((predictions1[i] + predictions2[i] + predictions3[i] + predictions4[i] + predictions5[i] + predictions6[i] + predictions7[i] + predictions8[i] + predictions9[i]) / 9), tickers[i]]
    ...
    # Print Results
    for i in range(len(tickerPredictList)):
        print(f"Ticker: {tickerPredictList[i][1]}  --  Predicted Change: {tickerPredictList[i][0]}")

    return tickerPredictList
