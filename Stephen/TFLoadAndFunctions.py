...
def PredictList(TickerList, GainLossThreshold):
    ...
    for i in range(len(gainLossList)):
        if gainLossList[i][1] > GainLossThreshold:                    
            # Create a dataset for ticker
...
def DataFrameFromData(dataSet):    
    ...
    # Normalizes Data
    # mean = "30.953331029999937, 31.59265121481317, 30.268035978848367, 30.930063361188058, 0.7846987415601376, 0.616695790170232, 0.9490855376038697, -4.897895495031242, -4.890292855176936, -4.942769211850064, -4.830491078487062, 6.783618497719114, 6.785513945902896, 6.744851112919943, 7.186903207294147, 1.613891442726054, 1.6072546803539325, 1.5965530225394837, 1.9094176275275665, 0.3802174307682398, 0.38134932393260995, 0.3813079810502029, 0.39030398144840633, -0.42637693465365095, -0.410768883917853, -0.4084288533498861, -0.4078465394367765, 50.2502773369562"
    # meanData = pd.read_csv(StringIO(mean))
    # std = "48.228135132358645, 49.293865599270966, 47.04880459587488, 48.1206957495542, 8.945184274775398, 11.68328227060429, 15.778014885582117, 14.91593843762734, 14.966049206934262, 15.157647825798241, 16.13522622020681, 22.536451442684175, 22.508793093459772, 22.526683034701364, 23.913434610766934, 26.62683070646213, 26.607339956635766, 26.609220588133354, 27.48752246054908, 2.302201259863434, 2.302602396899637, 2.302596284018156, 2.3186034555316675, 3.047117509241349, 2.9482342130549046, 2.946903503701804, 2.9469257113402727, 47.61862422593376"
    # stdData = pd.read_csv(StringIO(std))
    # df = (df - meanData) / stdData
...
def BubbleSort(SortingList):
    ...
    for i in range(len(SortingList)):
        for j in range(len(SortingList) - 1):
            if SortingList[j][0] < SortingList[j+1][0]:
                temp = SortingList[j]
