...
def GenerateFile(tickers, dateRange, startDateTime):    
        if len(tickers) > 0:
            for thisTicker in range(len(tickers)):
                tickerName = tickers[thisTicker] 
                startTime = datetime.now()
                printTime = startTime.time()
                print(f"Start Time: {printTime}")
                row = [None] * 35
                currentYear = 0
                fileName = f"DataSets\dataSet_{tickerName}.csv" #Creates a new file for each year to break file into bite-sized segments
...
dateRange = (365 * 5) # + (365 * 5) # Runs for 1 year or until there is not enough information to continue
tickers = GetTickers()
